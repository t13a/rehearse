...
workspace/data/c/        owner: agent,   mode 755   → 中身は自由にいじれる
workspace/data/d/        owner: agent,   mode 755   → 中身は自由にいじれる
```

`data/` の write 権限を落とすことで、 `a`/`b` の symlink 自体を `mv` で剥がされるのを防ぐ。 `c/` `d/` の内部では agent が自由に動ける。 workspace ルート (`meta.json`, `commit.log`, `.git/`) は container に一切マウントされないので、 agent からは観測不能。

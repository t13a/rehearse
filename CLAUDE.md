...
- agent コンテナは `REHEARSE_AGENT_UID` (既定 10000) で起動するため、 `inbox/` の symlink や agent が `outbox/` に持ち込んだ symlink は **harness UID からは直接削除できない**。 cleanup はすべて `rehearse purge` 経由 (内部で root コンテナ越しに `rm -rf`)

...
2. ✅ **最小ハーネスのコード** — Python + uv + argparse。 CLI 6 コマンド (`create` / `run` / `status` / `discard` / `purge` / `commit`)。事前検証、 sticky bit 付き workspace 構築、 chown コンテナによる `c/` の agent UID ハンドオフ、 busybox placeholder の agent 実行、 git スナップショットまで。 pytest で lifecycle を end-to-end テスト

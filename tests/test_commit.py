...
def test_commit_moves_a_files(tmp_path: Path, fake_ab: tuple[Path, Path]) -> None:
    ...
    d = session_dir / "data" / "d"

    # Simulate agent moving c/file1.txt → d/newdir/file1.txt
    src_link = session_dir / "data" / "c" / "file1.txt"
    dst_dir = d / "newdir"
    dst_dir.mkdir()
...
def test_commit_skips_fyi_files(
    ...
    (d / "note.FYI.md").write_text("Some notes\n")

    # Also move an A-origin file
    src_link = session_dir / "data" / "c" / "file1.txt"
    src_link.rename(d / "file1.txt")
...
def test_commit_aborts_on_conflict(
    ...
    # Agent moves c/file1.txt → d/collision/file1.txt (same name as B's existing)
    src_link = session_dir / "data" / "c" / "file1.txt"
    src_link.rename(d / "collision" / "file1.txt")

...
- Codex CLI の認証情報 (`OPENAI_API_KEY` または skeleton で持ち込む `.codex/auth.json`)
- `ANTHROPIC_API_KEY` (Claude Code agent を使う場合のみ。テスト実行には不要)
...
事前に image をビルドし、Codex CLI の認証情報を用意する。ChatGPT login cache を使う場合は [Home skeleton](#home-skeleton) で `.codex/auth.json` を持ち込む:
...
| `OPENAI_API_KEY` | (未設定) | Codex runner が設定されていれば container に pass-through する。 `.codex/auth.json` を使う場合は不要 |
| `ANTHROPIC_API_KEY` | (未設定) | Claude Code runner が必須として要求。 host の環境変数から container に pass-through される |

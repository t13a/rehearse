...
uv run rehearse run "$SID"            # Claude Code 起動。終わると archive/.done が生える
uv run rehearse status "$SID"
ls ~/.local/share/rehearse/sessions/"$SID"/data/archive/
(cd ~/.local/share/rehearse/sessions/"$SID" && git status)
uv run rehearse commit "$SID"         # archive/ の配置に従って A→B にファイル移動
...
stat -c '%a %U:%G %n' ~/.local/share/rehearse/sessions/$SID/data/archive
```

- `inbox/` は agent UID (既定 10000:10000) 所有
- `archive/` とサブディレクトリは `1777` (sticky、 harness 所有)
...
agent コンテナが UID 10000 で動くため、 `inbox/` の symlink や agent が `archive/` に持ち込んだ symlink は**通常ユーザーからは `rm` できない**。 workspace を消すときは必ず:
